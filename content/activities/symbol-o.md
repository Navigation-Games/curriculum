...
Learn map symbols and the features they represent in the terrain. Use a clue sheet to visit different features within a boundary. Repeat using different clue sheets to learn all the symbols.
...
- Short: Matching symbols to features
  Long: Match symbols on the map to features on the ground

- Short: Symbol clue sheets
  Long: Use a symbol clue sheet to find checkpoints in order (similar to Animal-O, but with symbols instead of animal pictures)

- Short: Self-checking
  Long: Self-check using the animal pictures at each checkpoint
...
1. Short: Run the boundary
   Long: Run the boundary, discuss inside/outside
2. Short: Explore to find checkpoints
   Long: Explore to find checkpoints, remembering where a few of them are. Gather on the signal
3. Short: Learn the orienteering map
   Long: Compare the orienteering map to any picture maps or pattern maps used before. Orient the map. Point out features and have volunteers run to them and back
4. Short: Learn the map symbols
   Long: Show the map key. If possible, hide the text labels and have students guess what each symbol represents before revealing it. Quiz students on matching symbols to features and features to symbols
5. Short: Visit checkpoints matching the symbols in order
   Long: Point to a symbol on the key and challenge all students to find the checkpoint at that type of feature. Then pass out symbol clue sheets. Students work individually or in pairs, visiting features in the order the symbols appear on the sheet. They check their answer using the animal picture at each checkpoint
6. Short: Check and repeat
   Long: When students finish, verify they visited all features in the correct order. If they made mistakes, explain where the error was and let them try again. Students may repeat courses as they please
